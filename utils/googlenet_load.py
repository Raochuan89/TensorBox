...
from slim_nets import  andreanet
...
def model(x, H, reuse, is_training=True):
    ...
    if H['slim_basename'] == 'resnet_v1_101':
        ...
        with slim.arg_scope(andreanet.andreanet_arg_scope()):
            _, T = andreanet.andreanet(x, is_training=is_training, reuse=reuse)
    feat = T[H['slim_top_lname']]
    return feat
