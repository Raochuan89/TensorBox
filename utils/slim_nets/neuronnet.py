...
def andreanet(images, num_classes=2, is_training=False,
  ...
  with tf.variable_scope(scope, 'AndreaNet', [images], reuse=reuse):
    net = end_points['conv1'] = slim.conv2d(images, 32, [3, 3], scope='conv1', stride=2)
    net = end_points['conv2'] = slim.conv2d(net, 32, [3, 3], scope='conv2', stride=2)
    ...
    net = end_points['dense1'] = slim.conv2d(net, 256, [2, 2], scope='dense1', activation_fn=tf.nn.tanh)
    net = end_points['dropout3'] = slim.dropout(
        net, dropout_keep_prob, is_training=is_training, scope='dropout3')
    net = end_points['dense2'] = slim.conv2d(net, 1, [1, 1], scope='dense2', activation_fn=tf.nn.tanh)
    net = slim.flatten(net)
    end_points['Flatten'] = net

    logits = end_points['Logits'] = slim.fully_connected(net, 9, scope='fc3')
  ...
  return logits, end_points
andreanet.default_image_size = 64
